...
| **ContentStudio**       | **Operational Content Workspace** — standalone governed platform for creating, reviewing, approving, publishing, and versioning institutional content. NOT a subsystem of LocalContentOS. Has its own Prisma models (ContentWorkspace, ContentItem, ContentVersion, ContentTemplate), 5 route groups, full lifecycle (DRAFT→IN_REVIEW→APPROVED→PUBLISHED→ARCHIVED), versioning, PDF export, audit trail, and ~125 tests. Current maturity: L4 Usable v0.1. See AQLIYA_SYSTEM_TAXONOMY.md for scope definition. |
