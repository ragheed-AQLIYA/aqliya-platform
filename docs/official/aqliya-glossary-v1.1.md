...
**Note:** Product/system definitions updated to reflect L5 Pilot-ready (conditional) status per P0 governance freeze (ADR-109). DecisionOS remains L6 Production-hardened.
**ملاحظة:** تم تحديث تعريفات المنتجات/الأنظمة لتعكس حالة L5 الجاهزة للتجربة بشروط pursuant to P0 governance freeze. DecisionOS يبقى L6.
...
| **WorkflowOS** | **سير العمل — نظام سير العمل** | Canonical governed custom/client-specific workspace at `/workflowos/*` (L5 Pilot-ready conditional). Real CRUD, workflow states, audit trail, PDF export. — مساحة عمل محكومة مخصصة/خاصة بالعميل في `/workflowos/*`. إنشاء وقراءة وتحديث وحذف حقيقي، حالات سير عمل، سجل تدقيق، تصدير PDF. |
...
| **SalesOS** | **مبيعات — نظام المبيعات** | Governed commercial intelligence workspace at /sales/* (L5 Pilot-ready conditional). Pipeline, deals, accounts, ICP, intelligence, audit trail, seed data. — مساحة عمل ذكاء تجاري محكوم في `/sales/*`. خط أنابيب، صفقات، حسابات، ملف العميل المثالي، ذكاء، سجل تدقيق، بيانات تمهيد. |
...
| **LocalContactOS** | **العلاقات — نظام العلاقات** | Governed relationship workspace at /contacts/* (L5 Pilot-ready conditional). Contact registry, sensitivity levels, interaction history, risk flags, compliance export, audit trail. — مساحة عمل علاقات محكومة في `/contacts/*`. سجل جهات الاتصال، مستويات الحساسية، سجل التفاعلات، إشارات المخاطر، تصدير الامتثال، سجل تدقيق. |
| **ContentStudio** | **استوديو المحتوى** | **Operational Content Workspace** — standalone governed platform for creating, reviewing, approving, publishing, and versioning institutional content. Has own Prisma models, 5 route groups, full lifecycle (DRAFT→IN_REVIEW→APPROVED→PUBLISHED→ARCHIVED), versioning, PDF export, audit trail, ~125 tests. L5 Pilot-ready (conditional). — **مساحة عمل المحتوى التشغيلي** — منصة محكومة مستقلة لإنشاء ومراجعة واعتماد ونشر وإصدار المحتوى المؤسسي. لديها نماذج Prisma خاصة، 5 مجموعات مسارات، دورة حياة كاملة، إصدارات، تصدير PDF، سجل تدقيق. |
...
| **RiskOS** | **المخاطر — نظام المخاطر** | AuditOS-adjacent risk workspace at /risk/* (L5 Pilot-ready conditional). Dashboard, assessment detail, procedure tracking, audit trail, JSON export. Not marketed as standalone product. — مساحة عمل مخاطر مجاورة لأوديت في `/risk/*`. لوحة معلومات، تفاصيل التقييم، تتبع الإجراءات، سجل تدقيق، تصدير JSON. لا تسوق كمنتج مستقل. |
...
| **Institutional Memory** | **الذاكرة المؤسسية** | Cross-product entity linking workspace at /institutional-memory/* (L5 Pilot-ready conditional). Knowledge graph visualization, event linking, collections, export. — مساحة عمل ربط الكيانات عبر المنتجات في `/institutional-memory/*`. تصور الرسم البياني المعرفي، ربط الأحداث، مجموعات، تصدير. |
