# AQLIYA Website Content Rewrite v2 — Final Merged Version

> **Status:** Final merged rewrite proposal. Ready for human approval, then implementation. Not yet implemented in source code.
> **Based on:** `docs/content/website-content-extract.md` + official v1.1 docs + merged review of v1 drafts
...
**Medium paragraph:**
عقلية هي منصة تشغيل مؤسسية تبني خطوط أنظمة ذكية فوق نواة حوكمة واحدة. كل مخرج مربوط بالدليل، كل خطوة تحتاج مراجعة بشرية، وكل قرار موثق في سجل تدقيق قابل للمراجعة. السحابة متاحة الآن. الخوادم الخاصة / On-Prem مسار استراتيجي مستقبلي، وليست حزمة إنتاجية متاحة حاليًا.
...
| **DecisionOS** | نظام مجاور — نشط | Adjacent active system | Presented as active workspace built on AQLIYA Core, not as a primary product line. Status badge: "نظام مجاور — نشط" |
...
| **Custom Systems / الأنظمة المؤسسية المخصصة** | يُفعّل حسب نطاق المؤسسة | Activated per institutional scope | Presented as built on AQLIYA Core. Studio is future; current offering is custom project-based. |
...
- Body: `تشغيل عقلية داخل البنية التحتية للمؤسسة مع قاعدة بيانات محلية وتحكم كامل. قيد التطوير، وسقد يتاح للمؤسسات الحساسة بعد اكتماله واعتماده.`
...
- DecisionOS badge changed from `نظام قائم` to `نظام مجاور — نشط` to align with v1.1 taxonomy
...
- خوادم خاصة — قيد التخطيط والتحضير (`/products`)
...
| `src/app/(marketing)/products/page.tsx` | DecisionOS badge (`نظام قائم` → `نظام مجاور — نشط`), product descriptions alignment | **High** | Status badges need v1.1 alignment |
...
Use this v2 document as the implementation source of truth for website copy. It keeps the stronger OpenCode structure while applying stricter public-claims discipline.
...
*End of final merged rewrite proposal. No source code files were modified or created besides this document.*
