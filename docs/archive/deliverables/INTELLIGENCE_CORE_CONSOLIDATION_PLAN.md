...
| Capability | Current Location | Decision | Rationale |
|------------|-----------------|----------|-----------|
| Decision Engine | `src/lib/decision/decision-engine.ts` | EXTRACTED ✅ → `src/lib/core/decision/engine.ts` + adapter | Generic multi-criteria engine with DecisionOS adapter |
| Decision contracts | `src/lib/decision/decision-type-config.ts` | KEEP (product) | DecisionOS-specific config |
| Decision intake | `src/lib/decision/intake.ts` | EXTRACTED ✅ → `src/lib/core/decision/evaluators/intake.ts` | Pure evaluation, no product imports |
| Decision framework | `src/lib/decision/framework.ts` | EXTRACTED ✅ → `src/lib/core/decision/evaluators/framework.ts` | Pure evaluation, no product imports |
| Decision scenarios | `src/lib/decision/scenarios.ts` | EXTRACTED ✅ → `src/lib/core/decision/evaluators/scenarios.ts` | Pure evaluation, no product imports |
| Decision risk-analysis | `src/lib/decision/risk-analysis.ts` | EXTRACTED ✅ → `src/lib/core/decision/evaluators/risk-analysis.ts` | Pure evaluation, no product imports |
| Decision recommendation | `src/lib/decision/recommendation.ts` | EXTRACTED ✅ → `src/lib/core/decision/evaluators/recommendation.ts` | Pure evaluation, no product imports |
| Decision-specific | `src/lib/decision/sector*.ts, outcome*.ts` | KEEP (product) | DecisionOS-specific |

**Steps:**
1. ~~Create `src/lib/core/decision/` directory~~ ✅ Done
2. ~~Extract 5 evaluators (intake, framework, scenarios, risk-analysis, recommendation)~~ ✅ Done
3. ~~Create generic DecisionEngine with interface-based config~~ ✅ Done
4. ~~Create DecisionOS adapter bridging product config → generic engine~~ ✅ Done
5. ~~Keep product-specific logic in DecisionOS~~ ✅ Done
6. ~~Wire DecisionOS to use core engine via re-exports~~ ✅ Done
7. ~~15 core tests + 54 decision tests all passing~~ ✅ Done
