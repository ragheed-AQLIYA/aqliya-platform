...
No markdown links `` `link-text`(AUDITOS_OPERATOR_MANUAL.md) `` or `` `link-text`(docs/systems/AUDITOS_OPERATOR_MANUAL.md) `` were found outside README table cells (which are plain text, not link syntax).
