...
>
> **Reality Hardening Pass (2026-05-28):** The table row for LocalContentOS below (line 31) is **outdated** and describes a pre-implementation state. LocalContentOS has 12 workspace routes at `/local-content/*`, server-action-backed mutations, seed data, evidence upload, review/approval, PDF/XLSX exports, and an audit trail. It is classified as L5 pilot-ready with conditions. Do not rely on this row for current status. See `docs/source-of-truth/PRODUCT_STATUS_MATRIX.md` or inspect `src/app/local-content/` for authoritative current status.
...
- What is misleading or overstated: some protected-looking pages are still mock/prototype (`src/app/(dashboard)/organizations/*`, `src/app/(dashboard)/settings/page.tsx`, `src/app/sales/page.tsx`); some API routes may expose downloads or metrics without visible route-level auth checks (partially hardened as of 2026-05-28).
- What should be considered the real current version: a mixed-state governed application with substantial implemented products/workspaces (`AuditOS` L5, `LocalContentOS` L5, `DecisionOS` L4, `Office AI Assistant` L4, `WorkflowOS` L4), and several shells/marketing surfaces around them (`SalesOS` L3, `SimulationOS` L1, `Organizations` L3, `Settings` L2/L4).
...
| Sunbul workspace              | `src/app/sunbul/**` (redirects to `/workflowos/*`), `src/actions/workflowos-actions.ts`, `src/lib/workflowos/**`, `prisma/schema.prisma` (`Sunbul*` models)                                                               |         N/A | Routes are `permanentRedirect(302)` to `/workflowos/*`. Sunbul data models are reused by WorkflowOS.         | Sunbul is a redirect alias. WorkflowOS is the canonical product name.                                          | Internal only (redirect)              |
| workflowos                    | `src/app/workflowos/**`, `src/actions/workflowos-actions.ts`, `src/lib/workflowos/**`, `prisma/schema.prisma` (`Sunbul*` models — reused)                                                                         |         A | Real governed workspace with CRUD, workflow states, audit trail, PDF export. Canonical product name. Sunbul is redirect alias to WorkflowOS. | None significant at L4 | Safe to show with explanation         |
...
| LocalContentOS                | `src/app/local-content/**` (12 routes), `src/actions/localcontent-actions.ts`, `src/lib/local-content/**`, `prisma/schema.prisma` (`LocalContent*` models — 8 models)                                            |         B | Real workspace with projects, suppliers, spend, classification, evidence, findings, review/approval, exports, audit trail, seed data, Arabic-first UI | Not L6 production-hardened; Arabic PDF font rendering is P2 | Safe to show with explanation         |
