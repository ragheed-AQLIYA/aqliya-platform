...
| `/products/local-content`                           | LocalContentOS product page          | Marketing              | Active — marketing only. Workspace routes at `/local-content/*` are now live (Phase 3: 5 routes implemented). |
...
8. Do not create `/simulation` top-level routes until that system has a real workspace implementation. `/local-content/*` workspace routes are planned and scope-locked for v0.1 — implementation may begin once the scope-lock report confirms readiness.
