...
| **CI/CD** | L5 | Multi-workflow pipeline: `ci.yml` (tsc, lint, test, build, prisma, guards, demo smoke, audit), `deploy.yml` (test → terraform plan → build & push → deploy → post-deploy smoke test), `promote.yml` (staging → production with environment approval gate), `backup.yml` (scheduled DB backup + verify). Consistent Node 22 across all workflows. `scripts/post-deploy-smoke.mjs` for post-deployment validation. `.github/workflows/`. | Test DB matrix |
| **Backup** | L5 | Production-grade scripts with safety gates (`CONFIRM_RESTORE`). `scripts/db-backup-scheduler.ts`, `scripts/db-restore-drill.ts`. | Scheduled automation, periodic restore drill |
