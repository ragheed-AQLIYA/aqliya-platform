...
> **هام:** هذا الإصدار تم تطويره باستخدام `prisma db push` في بيئة التطوير. الانتقال إلى الإنتاج يتطلب تنفيذ خطة الترحيل الموثقة في `docs/products/aqliya-production-migration-plan.md`. لا تستخدم `prisma db push` في الإنتاج.
