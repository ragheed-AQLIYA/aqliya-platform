...
export default function ReviewPage() {
  ...
  const getTargetLabel = (c: ReviewComment) => {
    return `${c.targetType}: ${c.targetId.substring(0, 8)}...`;
  };
      ...
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2">
          {(["all", "open", "resolved"] as const).map((f) => (
            ...
            >
              {f === "all"
                ? t("all", { count: comments.length })
                : f === "open"
                  ? t("open", { count: openCount })
                  : t("resolved", { count: comments.length - openCount })}
            </Button>
          ))}
        </div>
      </div>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center gap-2">
                <div className="flex-1">
                  ...
                  >
                    ...
                    <SelectContent>
                      <SelectItem value="statement_line">
                        {t("statementLine")}
                      </SelectItem>
                ...
                <div className="flex-1">
                  ...
                  <Select
                    ...
                    onValueChange={(v) => {
                      if (v !== null) setCommentTargetId(v);
                    }}
                    disabled={getTargetOptions().length === 0}
                  >
                    <SelectTrigger className="h-8 text-xs">
                      <SelectValue
                        placeholder={
      ...
      <div className="space-y-3">
        {filtered.map((comment) => (
          <ReviewCommentCard
            key={comment.id}
            comment={comment}
            targetLabel={getTargetLabel(comment)}
            onTrace={async (c) => {
              setTraceComment(c);
              try {
