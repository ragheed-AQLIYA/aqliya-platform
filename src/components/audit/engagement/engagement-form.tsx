...
export function EngagementForm({ open, onClose, onCreated, engagement, users = [], organizationId }: EngagementFormProps) {
  const formKey = `form-${open ? (engagement?.id ?? 'new') : 'closed'}`

  return (
    <Dialog key={formKey} open={open} onOpenChange={onClose}>
      <FormContent
        engagement={engagement}
        users={users}
...
function FormContent({
  ...
  const [clientName, setClientName] = useState(engagement?.client?.name || "")
  const [fiscalPeriod, setFiscalPeriod] = useState(engagement?.fiscalPeriod || "")
  const [engagementType, setEngagementType] = useState<string>(engagement?.engagementType || "full_audit")
  const [selectedUsers, setSelectedUsers] = useState<string[]>(engagement?.team?.map(m => m.userId) || [])
