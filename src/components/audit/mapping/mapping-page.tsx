...
export default function MappingPage() {
  ...
  useEffect(() => {
    ...
    getCanonicalAccountsAction()
      .then(setCanonicalAccounts)
      .catch(() => setCanonicalAccounts([]));
  }, [engagementId]);
  ...
  const handleManualMappingChange = async (
    mappingId: string,
    canonicalAccountId: string | null,
  ) => {
    const result = await updateManualMappingAction({
      engagementId,
      mappingId,
      canonicalAccountId,
    });
  ...
  if (filter === "mapped")
    filtered = mappings.filter((m) => m.status === "confirmed");
  if (filter === "unmapped")
    filtered = mappings.filter((m) => m.status === "pending");
  ...
  return (
    <div className="space-y-6" dir="rtl">
      {toast && (
        <div
          className={`fixed start-4 top-4 z-50 rounded-2xl px-4 py-2 shadow-lg text-sm font-medium text-white ${toastType === "success" ? "bg-green-600" : "bg-red-600"}`}
        >
          {toastType === "success" ? (
            <CheckCircle className="inline size-4 me-1" />
          ) : (
            <XCircle className="inline size-4 me-1" />
          )}
          {toast}
        </div>
      )}
              ...
              <TableBody>
                {filtered.map((m) => (
                  <TableRow key={m.id}>
                    ...
                    <TableCell>
                      {m.debitAmount > 0 ? sar(m.debitAmount) : "-"}
                    </TableCell>
                    <TableCell>
                      {m.creditAmount > 0 ? sar(m.creditAmount) : "-"}
                    </TableCell>
                    <TableCell>
                      {m.canonicalAccountName ? (
                        <span className="flex items-center gap-1">
                          {m.mappingType === "ai_suggested" && (
                            <Sparkles className="size-3 text-purple-500" />
                          )}
                          {m.mappingType === "confirmed_ai" && (
                    ...
                    <TableCell>
                      {m.status === "confirmed" ? (
                        ...
                        <Badge
                          variant="outline"
                          className="bg-amber-100 text-amber-700 border-amber-300 flex items-center gap-1 w-fit"
                        >
                    ...
                    <TableCell>
                      <div className="flex items-center gap-2">
                        ...
                        <Select
                          value={m.canonicalAccountId ?? ""}
                          onValueChange={(v) => {
                            handleManualMappingChange(m.id, v || null);
                          }}
                        >
