...
export function SunbulClientList({
  onSelectClient,
  onChange,
}: {
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    setCreating(true);
    setError(null);
    const r = await sunbul_createClient({
      name: newName.trim(),
      slug: newSlug.trim(),
    });
    setCreating(false);
  ...
  async function handleToggleStatus(client: ClientItem) {
    ...
    await sunbul_updateClientStatus(client.id, newStatus);
    load();
  }
