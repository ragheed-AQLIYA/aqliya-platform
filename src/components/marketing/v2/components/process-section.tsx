...
export function ProcessSection({ phases, principles, universalSteps, title, hint }: ProcessSectionProps) {
  return (
    <section id="process" className="scroll-mt-28 border-t bg-muted/10 py-16 sm:py-20">
      <div className="mx-auto max-w-7xl px-6">
        <h2 className="text-2xl font-black sm:text-3xl">{title}</h2>
        <p className="mt-2 text-sm text-muted-foreground">{hint}</p>
        <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {phases.map((phase, i) => (
            <Reveal key={phase.num} delay={i * 70} className="rounded-xl border border-border/60 bg-background p-5">
