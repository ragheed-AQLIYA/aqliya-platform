...
export function OperatingSystemMapVisual({ variant = "dark", className }: OperatingSystemMapVisualProps) {
  const isDark = variant === "dark"
  const bg = isDark ? "#0B1728" : "#F6F9FC"
    ...
    <div className={cn("relative w-full rounded-xl border p-4 sm:p-6", isDark ? "border-white/10 bg-[#0B1728]" : "border-border bg-muted/10", className)}>
      ...
      <div className={cn("flex items-center justify-between mb-4 pb-3 border-b", isDark ? "border-white/10" : "border-border")}>
        ...
        <div className="flex items-center gap-3 text-[10px]">
          <span className={cn("flex items-center gap-1", isDark ? "text-white/40" : "text-muted-foreground")}>
      ...
      <div className="flex flex-wrap items-center justify-center gap-2 sm:gap-3 mb-4">
        {mainNodes.map((node, i) => (
          <div key={node.id} className="flex items-center gap-2 sm:gap-3">
            ...
            >
              <div className="text-xs sm:text-sm font-semibold" style={{ color: textPrimary }}>{node.label}</div>
              <div className="text-[9px] sm:text-[10px]" style={{ color: textSecondary }}>{node.sub}</div>
            </div>
            {i < mainNodes.length - 1 && (
              <svg className="h-3 w-3 shrink-0 rtl:rotate-180" style={{ color: lineColor }} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                <path d="M5 12h14" />
      ...
      {/* Supporting Nodes */}
      <div className={cn("flex flex-wrap items-center justify-center gap-2 pt-3 border-t", isDark ? "border-white/10" : "border-border")}>
        {supportingNodes.map((node, i) => (
          <div
            key={i}
            className="rounded-full px-3 py-1 text-center transition-all hover:scale-[1.02]"
            style={{
              background: isDark ? "rgba(19,125,197,0.08)" : "rgba(19,125,197,0.06)",
              border: `1px dashed ${isDark ? "rgba(19,125,197,0.2)" : "rgba(19,125,197,0.15)"}`,
            }}
          >
            <div className="text-[10px] sm:text-xs font-medium" style={{ color: accentColor }}>{node.label}</div>
            <div className="text-[8px] sm:text-[9px]" style={{ color: textSecondary }}>{node.sub}</div>
          </div>
        ))}
      </div>
    </div>
