...
interface SystemFlowVisualProps {
  steps: string[]
  className?: string
}

export function SystemFlowVisual({ steps, className }: SystemFlowVisualProps) {
  return (
    <div className={cn("flex flex-wrap items-center justify-center gap-2 sm:gap-3", className)}>
      {steps.map((step, i) => (
        <div key={i} className="flex items-center gap-2 sm:gap-3">
          <div className="rounded-lg border bg-background px-3 py-2 text-sm font-medium shadow-sm sm:px-4 sm:py-2.5">
            {step}
          </div>
          {i < steps.length - 1 && (
