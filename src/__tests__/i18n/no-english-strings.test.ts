...
const ALLOWED_ENGLISH = new Set([
  ...
  "draft", "open", "closed", "pending", "resolved",
  "accepted", "rejected", "in_progress", "in_review",
  "suggested", "implemented", "under_review",
  "missing", "requested", "uploaded", "linked", "reviewed",
  "dismissed", "approved", "published", "archived",
  "setup", "awaiting_client", "ready_for_approval",
  "not_ready", "pending_approval", "blocked", "locked",
  "not_started", "changes_requested", "investigated",
  "conditional", "trusted", "sufficient", "partial", "conflicting", "weak",
  ...
  // -- User status & roles --
  "active", "inactive",
  "admin", "operator", "reviewer", "partner", "viewer",

  // -- Account types --
  "asset", "liability", "equity", "revenue", "expense", "non-current-asset",

  // -- Engagement types --
  "full_audit", "agreed_upon_procedures",
  ...
  // -- Tab / step keys --
  "overview", "trial-balance", "mapping", "validation", "statements",
  "notes", "evidence", "findings", "recommendations", "review",
  "approval", "publication", "audit-trail", "pilot",

  // -- Entity types --
  "engagement", "evidence", "finding", "recommendation",
  "publication", "event", "ai_output",
  "source_data", "account", "statement", "note",

  // -- Review filter values --
  "resolved",

  // -- Finding types --
  "material_misstatement", "control_deficiency", "disclosure_gap", "observation",

  // -- File extensions --
  "pdf", "xlsx", "docx", "jpg", "png", "csv",

  // -- Common record keys --
  "id", "label", "value", "key", "type", "name", "title", "description",
  "status", "role", "email", "phone",
  "forward", "backward",
...
const CSS_CLASS_REGEX = /^[a-z][a-z0-9\-:/[\]&._%]+(\s+[a-z][a-z0-9\-:/[\]&._%]+)*$/
...
function isEnglishUIString(candidate: string): boolean {
  ...
  if (words.length === 1) {
    ...
    if (/^[A-Z][a-z]+(?:[A-Z][a-z]+)*$/.test(trimmed) && words[0].length > 1 && !/[a-z]{2,}/.test(trimmed)) {
...
describe("audit components should not have untranslated English UI strings", () => {
  ...
  if (ALLOW_WARNINGS) {
    test.skip("should have no untranslated English UI strings (warning mode)", runAssertions)
  } else {
