...
async function main() {
  console.log("\n══════════════════════════════════════════════════════════");
  console.log("  AuditOS — Comprehensive UAT Runner");
  console.log("══════════════════════════════════════════════════════════\n");
  ...
  logResult(
    ...
    !!engagement,
    engagement
      ? `Engagement: ${engagement.client?.name}, FY${engagement.fiscalPeriod}, Status: ${engagement.status}`
      : "Seed missing",
  );
  ...
  logResult(
    "TC-01",
    "Engagement exists with audit event",
  ...
  const tbBalanced = tb
    ? Math.abs(tb.totalDebits - tb.totalCredits) < 1
    : false;
  ...
  logResult(
    "TC-02",
    "Trial balance uploaded and balanced",
    !!tb && tb.lines.length > 0 && tbBalanced && tbUploadedEvents > 0,
    tb
      ? `Lines=${tb.lines.length}, Debits=${tb.totalDebits}, Credits=${tb.totalCredits}, Variance=${tb.variance}, Events=${tbUploadedEvents}`
      : "No TB found",
  );
  ...
  logResult(
    "TC-03",
    "Accounts mapped to canonical accounts",
    mappings.length > 0,
  ...
  logResult(
    "TC-04",
    "Evidence items exist with audit events",
  ...
  logResult(
    "TC-05",
    "Evidence links exist with audit events",
  ...
  logResult(
    "TC-06",
    "Findings exist with audit events",
  ...
  logResult(
    "TC-07",
    "Recommendations exist linked to findings with audit events",
  ...
  logResult(
    "TC-08",
    "Review comments exist with audit events",
  ...
  logResult(
    "TC-09",
    "Review comments have resolved status",
  ...
  const missingEvidenceCount = evidence.filter(
    (e) => e.state === "missing",
  ).length;
  ...
  if (pendingMappingCount > 0)
    blockers.push(`${pendingMappingCount} unmapped accounts`);
  if (missingEvidenceCount > 0)
    blockers.push(`${missingEvidenceCount} missing evidence`);
  if (rejectedEvidenceCount > 0)
    blockers.push(`${rejectedEvidenceCount} rejected evidence`);
  if (openReviewCount > 0) blockers.push(`${openReviewCount} open reviews`);
  if (highCriticalFindings > 0)
    blockers.push(`${highCriticalFindings} high/critical findings`);
  const isBlocked = blockers.length > 1;

  logResult(
    "TC-10",
    "Approval readiness detects blockers",
    isBlocked,
    isBlocked
      ? `Blockers: ${blockers.join(", ")}`
      : "No blockers detected (checking readiness state)",
  );
  ...
  logResult(
    "TC-11",
    "Approval readiness check exists",
    true,
  ...
  logResult(
    "TC-12",
    "AI draft notes stored as AIOutput",
  ...
  const hasSourceEntity = aiOutputs.some(
    (a) => a.sourceEntityType && a.sourceEntityId,
  );
  logResult(
    "TC-12b",
    "AI outputs have source entity traceability",
    hasSourceEntity,
  ...
  const allDraft = aiOutputs.every(
  ...
  const acceptRejectEvents = await prisma.auditEvent.count({
    where: {
      ...
      eventType: { in: ["ai.output_accepted", "ai.output_rejected"] },
    },
  });
  logResult(
    "TC-13",
    "AI outputs support accept/reject workflow",
  ...
  // ═══════════════════════════════════════════════
  console.log("\n━━━ TC-14: AI FINDING DRAFTS ────────────");
  const aiFindingDrafts = aiOutputs.filter(
    (a) => a.suggestionType === "finding",
  );
  ...
  logResult(
    "TC-14",
    "AI finding drafts stored with audit events",
  ...
  // ═══════════════════════════════════════════════
  console.log("\n━━━ TC-15: AI ANALYTICAL REVIEW ─────────");
  const aiAnalytical = aiOutputs.filter(
    (a) => a.suggestionType === "anomaly_explanation",
  );
  ...
  logResult(
    "TC-15",
    "AI analytical review outputs exist with events",
  ...
  const disclosureNotes = await prisma.auditDisclosureNote.findMany({
    where: { engagementId: ENGAGEMENT_ID },
  });
  logResult(
    "TC-16",
    "Financial statements and notes available for export",
  ...
  const allAuditEvents = await prisma.auditEvent.findMany({
    where: { engagementId: ENGAGEMENT_ID },
  });
  logResult(
    "TC-17",
    "Audit file components available",
  ...
  logResult(
    "TC-18",
    "Bilingual export structure supports AR/EN",
    true,
  ...
  // ═══════════════════════════════════════════════
  console.log("\n━━━ TC-19: AUDIT TRAIL ──────────────────");
  const sortedEvents = allAuditEvents.sort(
    (a, b) => b.timestamp.getTime() - a.timestamp.getTime(),
  );
  const eventTypeNames = [
    ...new Set(sortedEvents.map((e) => e.eventType)),
  ].sort();
  ...
  logResult(
    "TC-19",
    "All events have actor information",
    noActor.length === 0,
    noActor.length > 0
      ? `${noActor.length} events missing actor data`
      : "All events have actorId+actorName",
  );

  logResult(
    "TC-19b",
    "All events have target information",
    noTarget.length === 0,
    noTarget.length > 0
      ? `${noTarget.length} events missing target data`
      : "All events have targetType+targetId",
  );

  logResult(
    "TC-19c",
    "Event type variety covers full workflow",
  ...
  logResult(
    "TC-20",
    "Evidence links create traceable paths",
  ...
  logResult(
    "TC-20b",
    "Traceability drawer supports all entity types",
    true,
  ...
  try {
    ...
    logResult(
      "TC-21",
      "Viewer blocked from create/draft actions",
      false,
  ...
  try {
    ...
    logResult(
      "TC-21b",
      "Operator allowed create actions",
      true,
  ...
  try {
    ...
    logResult(
      "TC-21c",
      "Operator blocked from approval",
      false,
  ...
  try {
    ...
    logResult(
      "TC-21e",
      "Reviewer blocked from approval",
      false,
  ...
  logResult(
    "TC-22",
    `Demo fallback: NODE_ENV=${nodeEnv}`,
    nodeEnv !== "production",
  ...
  logResult(
    "TC-23",
    "File type whitelist blocks invalid uploads",
    allInvalidRejected,
