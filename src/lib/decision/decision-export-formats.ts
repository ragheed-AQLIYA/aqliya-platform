...
export function formatExportMarkdown(data: ExportData): string {
  ...
  if (data.recommendation) {
    ...
    lines.push(`**Published:** ${data.recommendation.publishedAt ? data.recommendation.publishedAt.toISOString() : "Not published"}`)
    lines.push(`**Visible:** ${data.recommendation.isClientVisible ? "Yes" : "No"}`)
    lines.push(`**From Snapshot:** ${data.recommendation.publishedFromSnapshot ? "Yes" : "No"}`)
    lines.push("")
  ...
  if (data.approvedSnapshot) {
    ...
    lines.push("")
    lines.push(`**Source:** ${data.approvedSnapshot.isImmutable ? "Immutable" : "Legacy (Not Frozen)"}`)
    lines.push(`**Approved By:** ${data.approvedSnapshot.approver || "Unknown"}`)
    lines.push(`**Approved At:** ${data.approvedSnapshot.approvedAt ? data.approvedSnapshot.approvedAt.toISOString() : "Unknown"}`)
    lines.push("")
    lines.push("### Action")
    lines.push("")
    ...
    if (data.approvedSnapshot.confidence != null) {
      lines.push(`**Confidence:** ${Math.round(data.approvedSnapshot.confidence * 100)}%`)
      lines.push("")
    }
  ...
  if (data.approvalHistory.length > 0) {
    ...
    for (const a of data.approvalHistory) {
      lines.push(`| ${a.createdAt.toISOString()} | ${a.approver || "Unknown"} | ${a.status} | ${a.comments || "-"} |`)
    }
    lines.push("")
  }
  ...
  if (data.timeline.length > 0) {
    ...
    for (const e of data.timeline) {
      lines.push(`| ${e.date.toISOString()} | ${e.label}${e.isCritical ? " 🔴" : ""} | ${e.actor || "-"} | ${e.details || "-"} |`)
    }
    lines.push("")
  }
