...
describe("scoreDeal", () => {
  ...
  it("computes HEALTHY score for a strong deal", async () => {
    mockDealFindUnique.mockResolvedValue(
      makeDeal({ probability: 100, amount: 500000, stage: { slug: "closed_won", name: "Closed Won" } })
    )
  ...
  it("computes AT_RISK score for a weak deal", async () => {
    mockDealFindUnique.mockResolvedValue(
      makeDeal({ probability: 10, amount: 5000, stage: { slug: "drafting", name: "Drafting" } })
    )
    mockDealFindMany.mockResolvedValue([
      { amount: 100000 },
      { amount: 150000 },
  ...
  it("computes WATCH score for mid-range deal", async () => {
    mockDealFindUnique.mockResolvedValue(
      makeDeal({ probability: 60, amount: 100000, stage: { slug: "proposal", name: "Proposal" } })
    )
    mockDealFindMany.mockResolvedValue([
      { amount: 100000 },
      { amount: 150000 },
  ...
  it("computes stageScore correctly per stage slug", async () => {
    const stageTests = [
      ...
      { slug: "qualification", expected: 10 },
  ...
  it("caps score at 100 and floors at 0", async () => {
    ...
    const highResult = await scoreDeal("deal-1")
    expect(highResult.score).toBeLessThanOrEqual(100)
    expect(highResult.healthLevel).toBe("HEALTHY")
    ...
    const lowResult = await scoreDeal("deal-1")
    expect(lowResult.score).toBeGreaterThanOrEqual(0)
    expect(lowResult.score).toBe(0)
  })

  it("handles deal with no stage gracefully", async () => {
    mockDealFindUnique.mockResolvedValue(makeDeal({ stage: null }))
    mockDealFindMany.mockResolvedValue([{ amount: 100000 }])
  ...
  it("handles deal with no interactions", async () => {
    mockDealFindUnique.mockResolvedValue(makeDeal({ interactions: [] }))
    mockDealFindMany.mockResolvedValue([{ amount: 100000 }])
  ...
  it("handles zero amount and uses org average", async () => {
    mockDealFindUnique.mockResolvedValue(makeDeal({ amount: 0 }))
    mockDealFindMany.mockResolvedValue([{ amount: 100000 }])
...
describe("getDealHealth", () => {
  ...
  it("includes all sub-scores in result", async () => {
    mockDealFindUnique.mockResolvedValue(
      makeDeal({ stage: { slug: "negotiation", name: "Negotiation" } })
    )
    mockDealFindMany.mockResolvedValue([{ amount: 100000 }])
...
describe("listDealHealth", () => {
  ...
  it("returns health indicators for all open deals", async () => {
    mockDealFindMany
      .mockResolvedValueOnce([makeDeal({ id: "deal-1" }), makeDeal({ id: "deal-2" })])
...
describe("createForecast", () => {
  it("creates and returns a forecast with all fields", async () => {
...
describe("calculateForecast", () => {
  ...
  it("throws error for nonexistent forecast", async () => {
    await expect(calculateForecast("nonexistent")).rejects.toThrow("Forecast not found")
  })

  it("handles deals with null amounts and probabilities", async () => {
    const forecast = await createForecast("org-1", {
      name: "Test",
      period: "MONTHLY",
      periodStart: new Date("2026-01-01"),
...
describe("getPipelineAnalytics", () => {
  it("returns empty analytics when no deals", async () => {
    ...
    expect(Object.keys(result.dealsByStage).length).toBe(0)
  })
  ...
  it("computes correct pipeline totals", async () => {
    ...
    // weightedValue = 100000*0.8 + 200000*0.5 = 180000
  ...
  it("groups deals by stage", async () => {
    ...
    const deal2 = makeDeal({ id: "d2", stage: { slug: "negotiation", name: "Negotiation" } })
    mockDealFindMany
    ...
    expect(result.dealsByStage).toHaveProperty("qualifying")
    expect(result.dealsByStage).toHaveProperty("negotiation")
  })

  it("throws error for empty orgId", async () => {
    await expect(getPipelineAnalytics("")).rejects.toThrow("Organization ID is required")
  })
...
describe("getWinRateAnalysis", () => {
  ...
  it("extracts close reasons from metadata", async () => {
    mockDealFindMany.mockResolvedValue([
      makeDeal({ id: "d1", status: "closed_won", metadata: { closeReason: "Best value" } }),
      makeDeal({ id: "d2", status: "closed_lost", metadata: { closeReason: "Budget constraints" } }),
      makeDeal({ id: "d3", status: "closed_won", metadata: { closeReason: "Best value" } }),
    ])
  ...
  it("accepts custom period range", async () => {
    mockDealFindMany.mockResolvedValue([
      makeDeal({ id: "d1", status: "closed_won" }),
    ])
...
describe("getVelocityMetrics", () => {
  it("returns empty metrics when no closed deals", async () => {
    ...
    expect(Object.keys(result.monthlyTrend).length).toBe(0)
  })
  ...
  it("throws error for empty orgId", async () => {
    await expect(getVelocityMetrics("")).rejects.toThrow("Organization ID is required")
  })
})
// ─── Edge Cases ───

describe("edge cases", () => {
  it("handles deal with no stage gracefully in scoring", async () => {
    mockDealFindUnique.mockResolvedValue(makeDeal({ stage: null }))
    mockDealFindMany.mockResolvedValue([{ amount: 100000 }])
  ...
  it("handles deal with no interactions in scoring", async () => {
    mockDealFindUnique.mockResolvedValue(makeDeal({ interactions: [] }))
    mockDealFindMany.mockResolvedValue([{ amount: 100000 }])
  ...
  it("handles zero deal amount in value score", async () => {
    mockDealFindUnique.mockResolvedValue(makeDeal({ amount: 0 }))
    mockDealFindMany.mockResolvedValue([{ amount: 100000 }])
  ...
  it("handles zero probability in scoring", async () => {
    mockDealFindUnique.mockResolvedValue(makeDeal({ probability: 0 }))
    mockDealFindMany.mockResolvedValue([{ amount: 100000 }])
  ...
  it("handles legacy closed status as won in win rate", async () => {
    mockDealFindMany.mockResolvedValue([
  ...
  it("skips scoring failures in listDealHealth without crashing", async () => {
    mockDealFindMany.mockResolvedValue([makeDeal({ id: "d-fail" })])
    mockDealFindUnique.mockResolvedValue(null)
  ...
  it("handles null probability gracefully in forecast calculation", async () => {
    const forecast = await createForecast("org-1", {
      name: "Test",
      period: "MONTHLY",
      periodStart: new Date("2026-01-01"),
  ...
  it("updates weightedRevenue after calculation from null", async () => {
    const forecast = await createForecast("org-1", {
      name: "Calc Test",
      period: "MONTHLY",
