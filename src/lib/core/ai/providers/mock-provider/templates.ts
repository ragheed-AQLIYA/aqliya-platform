...
import { extractKeyContext } from "./common"

export function generateMockResponse(request: AIRequest): string {
  const { title, instructions, fileContent, isArabic, lang } = extractKeyContext(request)
