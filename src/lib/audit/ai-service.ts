...
export class AIService {
  ...
  static async generateSignals(): Promise<AiSuggestionResult[]> {
    await delay(800);
  ...
  static async draftRecommendation(
    findingId: string,
    findingDescription: string,
  ): Promise<AiSuggestionResult> {
    await delay(500);
    const suggestion = mockAiOutputs.find(
      (a) => a.suggestionType === "recommendation",
    )!;
    return {
      suggestion: {
        ...suggestion,
        id: `ai-rec-${Date.now()}`,
        inputContext: findingDescription,
        createdAt: new Date().toISOString(),
      },
      humanActionRequired: true,
      governanceMetadata: {
  ...
  static async draftNote(noteTitle: string): Promise<AiSuggestionResult> {
    await delay(400);
    const suggestion = mockAiOutputs.find(
      (a) => a.suggestionType === "note_draft",
    )!;
    return {
      suggestion: {
        ...suggestion,
        id: `ai-note-${Date.now()}`,
        inputContext: `Note: ${noteTitle}`,
        createdAt: new Date().toISOString(),
      },
      humanActionRequired: true,
      governanceMetadata: {
        ...
        modelVersion: "audit-os-llm-v1",
        requiresConfirmation: true,
        evidenceTraceAvailable: false,
      },
    };
  }

  // ─── Summarize Evidence ───
  static async summarizeEvidence(
    evidenceId: string,
    filename: string,
  ): Promise<AiSuggestionResult> {
    await delay(300);
    const suggestion = {
      ...
      engagementId: "",
      suggestionType: "evidence_summary" as const,
      inputContext: `Evidence: ${filename}`,
      outputContent: `Summary of ${filename}: This document contains supporting evidence for the related account balance. Key details include transaction references, dates, and amounts that corroborate the recorded figures.`,
      confidence: 0.85,
      modelVersion: "audit-os-llm-v1",
