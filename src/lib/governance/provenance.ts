import { ProvenanceMetadata, GovernanceTaskType, EvidenceStatus, ApprovalState, EscalationLevel, OutputBoundary, EscalationRecord, DoctrineReference, GovernanceReference, EvidenceRequirement } from './runtime-types';

export function createDraftProvenance(params: { taskType: GovernanceTaskType; doctrineReferences: DoctrineReference[]; governanceReferences: GovernanceReference[]; evidenceRequirements: EvidenceRequirement[]; reviewRequired?: boolean }): ProvenanceMetadata {
...
export function attachEvidenceStatus(metadata: ProvenanceMetadata, evidenceRequirements: EvidenceRequirement[]): ProvenanceMetadata {
  return { ...metadata, evidenceRequirements };
}

export function markReviewRequired(metadata: ProvenanceMetadata, reason: string): ProvenanceMetadata {
  return {
    ...metadata,
...
export function markEscalated(metadata: ProvenanceMetadata, record: EscalationRecord): ProvenanceMetadata {
  return {
    ...metadata,
...
export function markApprovedByHuman(metadata: ProvenanceMetadata, reviewer: string): ProvenanceMetadata {
  return {
    ...metadata,
...
export function explainProvenance(metadata: ProvenanceMetadata): string {
  ...
  parts.push(`Human approval: ${metadata.humanApprovalRequired ? 'required' : 'obtained'}`);
  if (metadata.approvedBy) parts.push(`Approved by: ${metadata.approvedBy}`);
  if (metadata.escalationLevel !== 'none') parts.push(`Escalation: ${metadata.escalationLevel}`);
  parts.push(`\nDoctrine references: ${metadata.doctrineReferences.map(d => `${d.documentId} (${d.principle})`).join('; ')}`);
  parts.push(`Governance references: ${metadata.governanceReferences.map(g => g.rule).join('; ')}`);
