...
export default function SalesOpportunityDetailError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    clientLogger.error("[SalesOS Opportunity Detail Error Boundary]", error);
  }, [error]);
      ...
      <Card className="border-dashed border-red-200 dark:border-red-900/50">
        <CardContent className="flex flex-col items-center justify-center space-y-4 p-8 text-center">
          ...
          <div className="space-y-1">
            <h2 className="text-lg font-semibold">تعذر تحميل تفاصيل الفرصة</h2>
            <p className="max-w-md text-sm text-muted-foreground">
              حدث خطأ أثناء تحميل تفاصيل الفرصة. يمكنك إعادة المحاولة أو العودة
              إلى قائمة الفرص.
            </p>
          </div>
          <div className="flex gap-2">
            ...
            <Link href="/sales/opportunities">
              <Button size="sm">العودة للفرص</Button>
            </Link>
          </div>
        </CardContent>
      </Card>
