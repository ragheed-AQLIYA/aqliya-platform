...
export default function SalesDealNewError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    clientLogger.error("[SalesOS Deal New Error Boundary]", error);
  }, [error]);
      ...
      <Card className="border-dashed border-red-200 dark:border-red-900/50">
        <CardContent className="flex flex-col items-center justify-center space-y-4 p-8 text-center">
          ...
          <div className="space-y-1">
            <h2 className="text-lg font-semibold">تعذر تحميل إنشاء الصفقة</h2>
            <p className="max-w-md text-sm text-muted-foreground">
              حدث خطأ أثناء تحميل صفحة إنشاء الصفقة. يمكنك إعادة المحاولة أو العودة إلى الصفقات.
            </p>
          </div>
