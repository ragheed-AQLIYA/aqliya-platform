...
const phases = [
  { num: "01", title: "فهم العمل", desc: "نحلل طبيعة عمل المؤسسة: العمليات، المستخدمين، الصلاحيات، البيانات، والمخرجات المطلوبة.", output: "ملخص فهم العمل", participants: "فريق العقلية + أصحاب العلاقة", next: "تحليل البيانات" },
  ...
  { num: "03", title: "تصميم سير العمل", desc: "نصمم مسار العمل الجديد: من الإدخال إلى المعالجة إلى المخرجات.", output: "خريطة سير العمل", participants: "فريق العقلية + أصحاب العلاقة", next: "تصميم المنتج" },
  ...
  { num: "05", title: "بناء النظام", desc: "نبني النظام بتقنيات حديثة، مع تركيز على قابلية التوسع والتعديل.", output: "نظام قابل للتشغيل", participants: "فريق العقلية", next: "اختبار المخرجات" },
...
export default function HowWeWorkPage() {
  return (
    <div className="flex flex-col">
      ...
      {/* Detailed Phases — Dense Layout */}
      <section className="mx-auto max-w-7xl px-6 pb-16">
        <div className="grid gap-4 sm:grid-cols-2">
          {phases.map((phase) => (
            <div key={phase.num} className="group rounded-xl border bg-background p-5 shadow-sm transition-all hover:border-primary/20 hover:shadow-md">
              <div className="flex items-start gap-4">
                <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-primary/10 text-sm font-black text-primary">
                  {phase.num}
                </div>
                <div className="flex-1">
                  <h2 className="text-base font-bold">{phase.title}</h2>
                  <p className="mt-1.5 text-sm leading-6 text-muted-foreground">{phase.desc}</p>
                  <div className="mt-3 grid gap-2 grid-cols-3 text-xs">
                    <div className="rounded-md border bg-muted/20 px-3 py-1.5">
                      <span className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">المخرج</span>
                      <div className="mt-0.5 font-medium">{phase.output}</div>
                    </div>
                    <div className="rounded-md border bg-muted/20 px-3 py-1.5">
                      <span className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">المشاركون</span>
                      <div className="mt-0.5 font-medium">{phase.participants}</div>
                    </div>
                    <div className="rounded-md border bg-muted/20 px-3 py-1.5">
                      <span className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">التالي</span>
                      <div className="mt-0.5 font-medium">{phase.next}</div>
                    </div>
                  </div>
                </div>
              </div>
      ...
      {/* CTA */}
      <section className="mx-auto max-w-7xl px-6 pb-16 border-t pt-16">
        <EnterpriseCTA
