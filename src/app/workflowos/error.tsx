...
export default function WorkflowOSError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    clientLogger.error("[WorkflowOS Error Boundary]", error);
  }, [error]);
        ...
        <CardContent className="flex flex-col items-center justify-center space-y-4 p-8 text-center">
          ...
          <div className="space-y-1">
            <h2 className="text-lg font-semibold">
              تعذر تحميل مساحة WorkflowOS
            </h2>
            <p className="max-w-md text-sm text-muted-foreground">
              تعذر تحميل مساحة العمل الحالية. يمكنك إعادة المحاولة أو العودة إلى
              الصفحة الرئيسية للمساحة.
            </p>
          </div>
          <div className="flex gap-2">
            ...
            <Link href="/workflowos">
              <Button size="sm">العودة إلى WorkflowOS</Button>
            </Link>
          </div>
        </CardContent>
      </Card>
