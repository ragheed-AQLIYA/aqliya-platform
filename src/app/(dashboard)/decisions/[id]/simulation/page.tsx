...
import {
  ...
  runSimulationAndRecommendation,
    ...
    <div>
      ...
      <div className="mt-6">
        <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h2 className="text-xl font-black">محاكاة السيناريوهات</h2>
            <p className="text-sm leading-7 text-muted-foreground">
              {isTender
                ? "تسجيل مالي وقدرات ومخاطر خاص بالمنافسات"
                : config?.scenarioGuidance ||
                  "تسجيل قرار مدعوم بالبيانات عبر ثلاثة سيناريوهات"}
            </p>
          </div>
        ...
        {!isTender &&
          derivedScores &&
          derivedScores.missingInputs?.length > 0 && (
            <Card className="mb-6 rounded-[24px] border-amber-200 bg-amber-50 p-4 shadow-sm">
              ...
              <p className="mt-2 text-xs text-amber-600">
                جودة البيانات: {derivedScores.dataQuality}/100 — إكمال هذه
                المدخلات يُحسّن دقة التسجيل.
              </p>
            </Card>
          )}

        {!isTender && scoreDrivers && (
          <Card className="mb-6 rounded-[24px] border-border/70 p-4 shadow-sm">
            <h3 className="text-sm font-semibold mb-3">محرّكات التسجيل</h3>
            <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
              {Object.entries(scoreDrivers).map(
                ([key, driver]: [string, any]) => (
                  <div
                    key={key}
        ...
        {scenarios.length === 0 && !loading ? (
          <p className="text-muted-foreground">
            لا توجد نتائج محاكاة بعد. اضغط "تشغيل المحاكاة" لتوليد النتائج.
          </p>
        ) : (
          <div className="grid gap-6 md:grid-cols-3">
            {scenarios.map((s: any) => (
              <Card
                key={s.type}
                className="rounded-[24px] border-border/70 p-4 shadow-sm"
              >
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-lg font-semibold">
                    {s.type?.replace("_", " ") || "غير معروف"}
                  </h3>
                  {s.simulation && (
                    <Badge
                      variant={
                        s.simulation.overallDecisionScore >= 75
                          ? "default"
                          : s.simulation.overallDecisionScore >= 55
                            ? "secondary"
                            : "destructive"
                      }
                    >
        ...
        {recommendation && (
          <div className="mt-8">
            ...
            <Card className="rounded-[24px] border-border/70 p-4 shadow-sm">
