      ...
      <div className="mt-6">
        <div className="grid grid-cols-2 gap-4 text-sm mb-6">
          ...
          <div>
            ...
            {decision.organization?.name || "N/A"}
          </div>
          {decision.priority && (
            <div>
          ...
          {decision.targetDate && (
            <div>
              ...
              {new Date(decision.targetDate).toLocaleDateString()}
            </div>
          )}
        </div>
        ...
        <div className="space-y-4">
          ...
          <section className="rounded-lg border p-4">
            <div className="mb-3 flex items-center justify-between gap-3">
              <h2 className="text-lg font-semibold">
                أ-١٫٢ السيناريوهات والخيارات
              </h2>
              {intake.status !== "accepted" || !frameworkState.isComplete ? (
                <Badge variant="secondary">محظور</Badge>
              ) : (
                <Badge
                  variant={scenarioState.isComplete ? "default" : "secondary"}
                >
          ...
          <section className="rounded-lg border p-4">
            <div className="mb-3 flex items-center justify-between gap-3">
              ...
              !scenarioState.isComplete ? (
                ...
                <Badge
                  variant={
                    riskAnalysisState.isComplete ? "default" : "secondary"
                  }
                >
            ...
            {!recommendationState.isComplete ? (
              ...
              <div className="space-y-3 text-sm">
                ...
                <div>
                  ...
                  <p className="mt-1 whitespace-pre-wrap text-muted-foreground">
                    {decision.recommendation.expectedNextState}
                  </p>
                </div>
              </div>
            ) : (
          ...
          <DecisionEvidence decisionId={decision.id} />
            ...
            <section>
              ...
              <div className="text-sm grid grid-cols-2 gap-2">
                <div>
                  ...
                  {decision.tenderProfile.clientName}
                </div>
                <div>
                  <span className="text-muted-foreground">قيمة العقد:</span>{" "}
                  {decision.tenderProfile.estimatedContractValue?.toLocaleString()}{" "}
                  ريال
                </div>
                <div>
                  <span className="text-muted-foreground">المدة:</span>{" "}
                  {decision.tenderProfile.durationMonths} شهراً
                </div>
                <div>
                  <span className="text-muted-foreground">الهامش:</span>{" "}
                  {decision.tenderProfile.marginEstimate}%
                </div>
              </div>
            </section>
          )}
