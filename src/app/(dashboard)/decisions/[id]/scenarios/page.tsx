...
export default function DecisionScenariosPage({
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    if (!id || intake?.status !== "accepted" || !frameworkState?.isComplete)
      return;
    ...
    <div>
      ...
      <div className="mx-auto mt-6 max-w-4xl">
        <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h2 className="text-xl font-black">أ-١٫٢ السيناريوهات والخيارات</h2>
        ...
        {success && (
          <div className="mb-4 rounded-2xl bg-green-50 p-3 text-sm text-green-600">
            تم حفظ السيناريوهات بنجاح. ثلاثة سيناريوهات أو أكثر ترفع جاهزية
            المحاكاة.
          </div>
        )}
        ...
        {blocked ? (
          <section className="rounded-[24px] border border-border/70 p-4 shadow-sm">
            <div className="mb-3 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <h3 className="font-bold">السيناريوهات محظورة</h3>
              {intakeBlocked ? (
          ...
          <>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <Textarea
                        ...
                        placeholder="صِف هذا المسار المحتمل دون ترتيب أو تقييم"
                      />
                    </div>
                    <div>
                      <Label htmlFor={`assumptions-${index}`}>
                        الافتراضات *
                      </Label>
                      <Textarea
                        ...
                        placeholder="الافتراضات اللازمة لتحقّق هذا السيناريو"
                      />
                    </div>
                    <div>
                      <Label htmlFor={`expectedOutcome-${index}`}>
                        النتيجة المتوقّعة *
                      </Label>
                      <Textarea
                        id={`expectedOutcome-${index}`}
                        ...
                        placeholder="النتيجة الوصفية المتوقّعة، وليست توصية"
                      />
                    </div>
                    <div>
                      <Label htmlFor={`affectedStakeholders-${index}`}>
                        أصحاب المصلحة المتأثرون *
                      </Label>
                      <Textarea
                        id={`affectedStakeholders-${index}`}
                        value={scenario.affectedStakeholders}
                        onChange={(event) =>
                          handleScenarioChange(
                            index,
                        ...
                        placeholder="أصحاب المصلحة المتأثرون بهذا المسار"
                      />
                    </div>
                    <div>
                      <Label htmlFor={`requiredConditions-${index}`}>
                        الشروط المطلوبة *
                      </Label>
                      <Textarea
                        id={`requiredConditions-${index}`}
                        value={scenario.requiredConditions}
                        onChange={(event) =>
                          handleScenarioChange(
                            index,
                            "requiredConditions",
                        ...
                        placeholder="الشروط اللازمة ليكون هذا السيناريو محتملاً"
                      />
                    </div>
                  </div>
                </section>
              ...
              <Button type="submit" disabled={saving}>
                {saving ? "جارٍ الحفظ..." : "حفظ السيناريوهات"}
              </Button>
            </form>
          </>
        )}
