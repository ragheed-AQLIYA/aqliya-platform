...
export default async function DecisionsPage() {
  ...
  return (
    <div className="space-y-6" dir="rtl">
      {/* Workspace Status */}
      <WorkspaceStatus module="decision" status="healthy" message="جميع مسارات العمل التشغيلية" />

      {/* Page Header */}
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-h2 font-bold text-foreground">DecisionOS</h1>
      ...
      {metrics && (
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          ...
          <KPICard
            ...
            changeType={metrics.avgCompletion > 70 ? "positive" : "warning" as any}
            icon={Brain}
            module="decision"
          />
        </div>
      ...
      <IntelligenceSummaryPanel
        ...
        signals={[
          ...
          { type: "confidence", label: "جودة الأدلة", value: "high", confidence: 0.75 },
          { type: "readiness", label: "توافق أصحاب المصلحة", value: "needs-review" },
        ]}
      />

      {/* AI Insight */}
      <AIInsightCard confidence={0.78}>
        قراران جاهزان للاعتماد. قرار واحد يتطلب تحليل سيناريوهات إضافي.
        يُوصى بمراجعة تحليل المخاطر للقرارات المعلّقة قبل المتابعة.
      </AIInsightCard>
        ...
        <EnterpriseCard>
          <EnterpriseCardContent className="py-12">
            <div className="text-center">
              ...
              <h3 className="text-base font-semibold text-foreground">لا توجد قرارات بعد</h3>
              <p className="text-sm text-muted-foreground mt-1">أنشئ أول قرار لبدء الحوكمة المنظّمة للقرارات.</p>
              ...
              <EnterpriseCard module="decision" hover className="h-full">
                <EnterpriseCardHeader>
                  <div className="flex items-start justify-between gap-2">
                    <EnterpriseCardTitle className="truncate">{decision.title}</EnterpriseCardTitle>
                    <StatusBadge status={getStatusBadgeStatus(decision.status)} size="sm" />
                  </div>
                </EnterpriseCardHeader>
                <EnterpriseCardContent>
                  <div className="flex flex-wrap gap-1.5 mb-2">
                    <Badge variant="outline" className="text-[10px]">{decision.type}</Badge>
                    {decision.priority && (
                      <Badge variant={getPriorityVariant(decision.priority)} className="text-[10px]">{decision.priority}</Badge>
                    )}
                  </div>
                ...
                <EnterpriseCardFooter className="flex items-center justify-between">
                  <Badge variant={getIntakeVariant(decision.intake?.status)} className="text-[10px]">
                    {decision.intake?.status?.replace("_", " ") || "استلام معلّق"}
                  </Badge>
                  <span className="text-[10px] text-muted-foreground">عرض التفاصيل ←</span>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <div className="lg:col-span-2">
          <SectionHeader eyebrow="النشاط" title="نشاط القرارات" description="أحداث مسارات القرارات الأخيرة" />
          <EnterpriseCard>
            <EnterpriseCardContent>
...
const mockRecentEntities = [
  { id: "1", type: "decision" as const, module: "decision" as const, title: "Q3 Investment Decision", status: "in_progress", accessedAt: new Date().toISOString(), href: "/decisions" },
  { id: "2", type: "engagement" as const, module: "audit" as const, title: "Acme Corp — FY2025", status: "active", accessedAt: new Date(Date.now() - 3600000).toISOString(), href: "/audit" },
  ...
  { id: "4", type: "decision" as const, module: "decision" as const, title: "Market Expansion", status: "approved", accessedAt: new Date(Date.now() - 86400000).toISOString(), href: "/decisions" },
]

const mockDecisionTimeline = [
  { id: "1", timestamp: new Date(), type: "ai" as const, title: "AI recommendation generated", description: "GO recommendation with 82% confidence", actor: "System" },
  { id: "2", timestamp: new Date(Date.now() - 3600000), type: "action" as const, title: "Scenario analysis completed", description: "3 scenarios evaluated", actor: "Ahmed M." },
  { id: "3", timestamp: new Date(Date.now() - 7200000), type: "status_change" as const, title: "Decision moved to review", description: "Framework complete → In Review", actor: "System" },
  { id: "4", timestamp: new Date(Date.now() - 86400000), type: "action" as const, title: "Risk analysis submitted", description: "All scenario risks documented", actor: "Sarah K." },
  { id: "5", timestamp: new Date(Date.now() - 172800000), type: "action" as const, title: "Decision created", description: "New decision workflow initiated", actor: "Ahmed M." },
]
