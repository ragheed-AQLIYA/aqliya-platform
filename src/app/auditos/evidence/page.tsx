              ...
              <div key={ev.id} className="flex items-start gap-3 rounded-lg border bg-background p-4">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {ev.fileType?.toUpperCase()} · {ev.state === "missing" ? "—" : `${(ev.fileSize / 1000).toFixed(0)} KB`}
                ...
                <span className={`shrink-0 rounded-full px-2 py-0.5 text-xs font-medium ${
                  ev.state === "accepted" ? "bg-green-50 text-green-700"
                    : ev.state === "reviewed" ? "bg-blue-50 text-blue-700"
            ...
            {findings.map((f) => (
              <div key={f.id} className="rounded-lg border bg-background p-4">
                ...
                <div className="mt-2 flex items-center gap-2 text-[10px] text-muted-foreground">
                  <span className="rounded bg-muted px-1.5 py-0.5">{f.findingType}</span>
                  <span>·</span>
                  <span>{f.materiality}</span>
                  {f.aiSuggested && <span className="text-purple-600">· AI</span>}
                </div>
                {recommendations
                  .filter((r) => r.findingId === f.id)
                  .map((r) => (
                    <div key={r.id} className="mt-2 rounded-md bg-muted/30 px-3 py-2">
                      <p className="text-xs font-medium text-primary">توصية: {r.title}</p>
                      <p className="mt-1 text-xs leading-5 text-muted-foreground">{r.recommendedAction}</p>
                    </div>
                  ))}
              </div>
            ))}
