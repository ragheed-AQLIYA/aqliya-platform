...
import { buildRecommendationDiff, getDiffSummary } from "@/lib/recommendation/recommendation-diff"
...
export async function getDecisionExportData(decisionId: string): Promise<{ success: boolean; data?: ExportData; error?: string }> {
  try {
    ...
    if (latestApproval) {
      if (hasImmutableSnapshot) {
        ...
        approvedSnapshot = {
          ...
          expectedNextState: latestApproval.recommendation?.expectedNextState || null,
          scopeExclusions: latestApproval.recommendation?.scopeExclusions || null,
          assumptionsUsed: latestApproval.recommendation?.assumptionsUsed || null,
    ...
    else if (!approvedSnapshot.isImmutable) warnings.push("Legacy approval — content not frozen, may have changed since approval")
    if (approvedSnapshot && decision.recommendation) {
      ...
      if (differs) warnings.push("Current recommendation differs from approved snapshot")
    }
    if (decision.recommendation?.isClientVisible && !approvedSnapshot?.isImmutable) {
    ...
    const exportData: ExportData = {
      ...
      recommendation: decision.recommendation
        ? {
            ...
            publishedFromSnapshot: decision.recommendation.publishedFromSnapshot,
