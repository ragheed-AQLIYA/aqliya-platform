// AuditOS — Backup Verification Helper
  ...
  try {
    ...
    for (const [label, table] of Object.entries(tables)) {
      ...
      if (count > 0) console.log(`  ${OK} ${label}: ${count} records`);
        ...
        console.log(`  ${FAIL} ${label}: 0 records — possible data loss`);
        allOk = false;
      }
    }
    ...
    if (allOk) {
      console.log(
        `  ${OK} Backup verification: PASS — all core tables have data`,
      );
