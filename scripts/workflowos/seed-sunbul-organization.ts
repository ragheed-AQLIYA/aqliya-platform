...
async function main() {
  ...
  console.log(
    `    WorkflowOS:  enabled  (active pilot — seeded via scripts/workflowos/seed-sunbul-pilot.ts)`,
  );
  console.log(
  ...
  if (!existingOrg) {
    ...
    console.log(`\n  Next steps:`);
    console.log(
      `    1. Run npx tsx scripts/workflowos/seed-sunbul-pilot.ts  — seed WorkflowOS pilot data`,
    );
